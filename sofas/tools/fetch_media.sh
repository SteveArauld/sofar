...
get() {
  ...
  for try in 1 2 3 4 5; do
    code=$(curl -s --compressed --http1.1 -A "$UA" -m 120 -o "$dst" -w "%{http_code}" "https://www.feiradossofas.pt/$rel")
...
if [ ! -s storage/app/media/videos/promo.mp4 ]; then
  curl -s --compressed --http1.1 -A "$UA" -m 180 -o storage/app/media/videos/promo.mp4 "https://www.feiradossofas.pt/videos/promo.mp4" && echo "ok   promo.mp4" || echo "FAIL promo.mp4"
fi
get "social.jpg"
