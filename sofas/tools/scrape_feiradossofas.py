...
# -*- coding: utf-8 -*-
"""
Scraper complet pour https://www.feiradossofas.pt/
...
Usage :
    python3 scrape_feiradossofas.py                 # tout
    python3 scrape_feiradossofas.py --limit 20      # test rapide sur 20 produits
    python3 scrape_feiradossofas.py --no-images     # métadonnées seulement
    python3 scrape_feiradossofas.py --workers 6     # parallélisme (défaut 5)
    python3 scrape_feiradossofas.py --img-size 1000-1000
"""
...
BASE = "https://www.feiradossofas.pt"
...
# --------------------------------------------------------------------------- #
#  2. Liste des produits (sitemap)
# --------------------------------------------------------------------------- #
def list_product_urls():
    print("→ Lecture du sitemap…")
    xml = fetch(SITEMAP_URL)
    if not xml:
        sys.exit("Sitemap illisible, abandon.")
    locs = re.findall(r"<loc>\s*([^<\s]+)\s*</loc>", xml)
    products = []
    for loc in locs:
        path = loc.replace(BASE, "").replace("https://feiradossofas.pt", "")
        m = PRODUCT_URL_RE.match(path)
        if m:
            products.append({"url": loc, "slug": path.lstrip("/"), "id": int(m.group(2))})
    # dédoublonnage par id
    uniq = {p["id"]: p for p in products}
    print(f"  {len(uniq)} produits listés.")
...
# --------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Scraper feiradossofas.pt")
